...
begin
  ...
  aProps := TODBCConnectionProperties.Create('','Driver=MySQL'+
   ...
   //      'Server=rootcode.info;Port=3306;USER=joni;Password=P@ssw0rd123','','');
      ...
      // aProps := TODBCConnectionProperties.Create('','Driver=Firebird'+
     //aProps := TODBCConnectionProperties.Create('','MySQL'+
      //     ';DSN=Firebird;DIRBER=Firebird;UID=SYSDBA;PWD=masterkey;FILEDSN=Firebird'+
      //     '','','');


 aProps := TSQLDBZEOSConnectionProperties.Create('zdbc:firebird-2.5://127.0.0.1:3050//fbdb/ErpDB.FDB?LibLocation=/usr/lib64/libfbclient.so;username=sysdba;password=masterkey','/fbdb/ErpDB.FDB','SYSDBA','masterkey');
